...
export default function AuthScreen({ navigation }: Props) {
  ...
  const handleSubmit = () => {
    ...
    if (password.length < 4) { Alert.alert('', '비밀번호를 4자 이상 입력해주세요'); return; }
    if (!isLogin && password !== passwordConfirm) { Alert.alert('', '비밀번호가 일치하지 않습니다'); return; }
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.authContainer}>
          ...
          <View style={styles.form}>
            ...
            {isLogin && (
              <View style={styles.options}>
                ...
                <Pressable onPress={() => Alert.alert('', '비밀번호 재설정 이메일을 보냈습니다')}>
          ...
          <View style={styles.socialRow}>
            <Pressable style={[styles.socialBtn, { backgroundColor: Colors.bgCard }]} onPress={() => handleSocialLogin('Google')}>
...
const styles = StyleSheet.create({
  ...
  blur1: { position: 'absolute', width: 156, height: 156, right: -20, top: -89, backgroundColor: 'rgba(241,232,207,0.3)', borderRadius: 78 },
  blur2: { position: 'absolute', width: 117, height: 117, left: -20, bottom: -45, backgroundColor: 'rgba(211,232,210,0.2)', borderRadius: 59 },
  ...
  brandIcon: {
    width: 64, height: 53, backgroundColor: Colors.bgInput, borderRadius: 32,
    alignItems: 'center', justifyContent: 'center',
  },

  authContainer: {
    backgroundColor: '#fff', borderRadius: 32, padding: 33,
    borderWidth: 1, borderColor: Colors.border, gap: 32,
    shadowColor: '#000', shadowOffset: { width: 0, height: 16 }, shadowOpacity: 0.03, shadowRadius: 32, elevation: 4,
  },
  toggle: { flexDirection: 'row', backgroundColor: Colors.bgCard, borderRadius: 9999, padding: 6 },
  toggleBtn: { flex: 1, paddingVertical: 10, borderRadius: 9999, alignItems: 'center' },
  toggleBtnActive: { backgroundColor: '#fff', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 },
  toggleText: { fontSize: 14, fontWeight: '500', color: Colors.textSecondary },
  toggleTextActive: { color: Colors.primary },

  form: { gap: 20 },
  formGroup: { gap: 8 },
  formLabel: { fontSize: 11, fontWeight: '500', color: Colors.textSecondary, textTransform: 'uppercase', letterSpacing: 1.1, paddingLeft: 16 },
  input: { backgroundColor: Colors.bgCard, borderRadius: 16, paddingHorizontal: 20, paddingVertical: 18, fontSize: 16, color: Colors.textPrimary },

  options: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 8 },
  checkbox: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  checkboxBox: { width: 20, height: 20, borderRadius: 6, borderWidth: 1, borderColor: Colors.borderMedium, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' },
  checkboxChecked: { backgroundColor: Colors.primary, borderColor: Colors.primary },
  checkboxLabel: { fontSize: 12, color: Colors.textSecondary },
  link: { fontSize: 12, color: Colors.primary },

  submitBtn: { backgroundColor: Colors.primary, borderRadius: 9999, paddingVertical: 16, alignItems: 'center' },
  submitText: { fontSize: 16, fontWeight: '500', color: Colors.primaryText },

  divider: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  dividerLine: { flex: 1, height: 1, backgroundColor: 'rgba(176,179,174,0.15)' },
  dividerText: { fontSize: 10, color: Colors.textMuted, textTransform: 'uppercase', letterSpacing: 1 },

  socialRow: { flexDirection: 'row', gap: 16 },
  socialBtn: { flex: 1, height: 56, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },

  footer: { textAlign: 'center', fontSize: 12, color: Colors.textSecondary, lineHeight: 19, marginTop: 24, paddingHorizontal: 16 },
});
